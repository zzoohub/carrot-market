      ...
      <div className="py-10 px-4">
        <div className="flex justify-center items-center space-x-3">
          <div className="relative w-16 h-16 rounded-full overflow-hidden bg-slate-500">
            <Image layout="fill" src={imgUrl(user?.avatar, "avatar")} className="object-cover" />
          </div>

          <div className="flex flex-col">
            <span className="font-bold text-gray-900 text-md">{user?.name}</span>
            <Link href={`/profile/edit`}>
              <span className="text-[12px] text-gray-700 cursor-pointer mt-1">Edit profile &rarr;</span>
            </Link>
          </div>
        </div>
          ...
          <Link href="/profile/onSales">
            <a>
              <div className="flex flex-col items-center py-3 hover:bg-slate-100">
                ...
                <span className="text-sm mt-2 font-medium text-gray-700">판매중</span>
              </div>
            </a>
          </Link>
        </div>
              ...
              <div>
                ...
                <div className="flex items-center">
                  {[1, 2, 3, 4, 5].map(star => (
                    <svg
                      key={star}
                      className={cls("h-5 w-5", review?.score! / 2 >= star ? "text-yellow-400" : "text-gray-300")}
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      aria-hidden="true"
