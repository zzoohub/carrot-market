...
const EditProfile: NextPage = () => {
  ...
  const [setProfile, { loading, data }] = useMutation<EditProfileResponse>(`/api/users/me`)
  ...
  const onValid = async ({ name, email, phone, avatar }: EditFormType) => {
    ...
    if (avatar && avatar.length > 0 && user) {
      ...
      form.append("file", avatar[0], `carrot_profileImg_userId=${String(user?.id)}`)
      const {
        result: { id },
  ...
  useEffect(() => {
    if (data && !data.ok && data.error) setError("formErrors", { message: data.error })
  ...
  return (
    <Layout seoTitle="Edit Profile" title="프로필 수정" canGoBack>
      ...
      <form onSubmit={handleSubmit(onValid)} className="py-10 px-4 space-y-5">
        <div className="flex items-center space-x-3">
          {avatarPreview ? (
            <div className="relative w-14 h-14 rounded-full bg-slate-500 overflow-hidden">
              <Image layout="fill" src={avatarPreview} className="object-cover" />
            </div>
          ) : (
        ...
        <Input register={register("name")} name="name" type="text" label="닉네임" kind="text"></Input>
        <Input register={register("email")} name="email" type="text" label="이메일" kind="text"></Input>
        <Input register={register("phone")} type="number" name="phone" label="전화번호" kind="phone"></Input>
        {errors.formErrors ? <em className="text-red-600 font-bold mt-3 block">{errors.formErrors.message}</em> : null}
        <Button onClick={() => clearErrors()} text={loading ? "Loading..." : "엄데이트"}></Button>
      </form>
    </Layout>
  )
}
